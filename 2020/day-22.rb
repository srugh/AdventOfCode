...
def solve_part_1(p1, p2)
  ...
  score = 0
  if p1.size > 0
    (0..p1.size-1).each_with_index do |idx|
      score += p1[idx] * (p1.size - idx)
    end
  else
    (0..p2.size-1).each_with_index do |idx|
...
def solve_part_2(p1, p2)

  _, deck = side_game(p1.dup, p2.dup)

  score = 0
 
    (0..deck.size-1).each_with_index do |idx|
...
def side_game(p1, p2)
    ...
    while p1.size > 0 && p2.size > 0 do
      ...
      if !start_mini_game?(c1, p1.size, c2, p2.size)
        ...
        winner, _ = side_game(p1[0..c1-1], p2[0..c2-1])
        if winner == "p1"
          p1 << c1
          p1 << c2
        else
...
part_1 = solve_part_1(p1.dup, p2.dup)
