...
def solve_part1(steps, p_text)
  action = ''
  sub_action = ''
  arg_1 = ''
  arg_2 = ''
  puts steps.size
  steps.each_with_index do |step, _count|
    case step[0]
    when 'swap'
    ...
    if action == 'swap' && sub_action == 'position'
      ...
      idx_1 = ''
      idx_2 = ''
      p_text.scan(/#{arg_1}/) do |_match|
        idx_1 = Regexp.last_match.begin(0)
      end
      p_text.scan(/#{arg_2}/) do |_match|
...
# solve_part1(input, p_text)
solve_part2(input.reverse, unscramble)
