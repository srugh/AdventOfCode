...
def solve_part1(fav_num, target)
  x, y = target
  grid = Array.new(1, [])
...
def bfs_grid(grid, y, x, target_y, target_x, fav_num)
  ...
  until queue.empty?
    ...
    return distance if current_y == target_y && current_x == target_x
    ...
    directions.each do |dr, dc|
      ...
      grid[new_y].push([]) if new_x > grid[new_y].size - 1

      if grid[new_y][new_x] != ['#'] && grid[new_y][new_x] != ['.']
        grid[new_y][new_x] = calc_loc(new_x, new_y, fav_num, grid)
      end

      next unless walkable.include?(grid[new_y][new_x]) && # Assuming 1 represents a traversable cell
...
def calc_loc(x, y, fav_num, _memo)
  ...
  calc_binary.count('1').even? ? ['.'] : ['#']
end

def pretty_print(grid)
  grid.each do |rows|
    rows.each do |col|
      val = col.first unless col.nil?
      val = '_' if val.nil?
      print val
    end
    print "\n"
...
solve_part1(fav_num, target)
solve_part2(fav_num, steps)
