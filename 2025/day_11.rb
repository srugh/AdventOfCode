...
def solve_part2(graph)
  ...
  svr_dac = fast_find(graph, 'svr', 'dac', %w[fft out].freeze, memo)
  svr_fft = fast_find(graph, 'svr', 'fft', %w[dac out].freeze, memo)
  dac_fft = fast_find(graph, 'dac', 'fft', %w[svr out].freeze, memo)
  fft_dac = fast_find(graph, 'fft', 'dac', %w[svr out].freeze, memo)
  fft_out = fast_find(graph, 'fft', 'out', %w[dac svr].freeze, memo)
  dac_out = fast_find(graph, 'dac', 'out', %w[svr fft].freeze, memo)
