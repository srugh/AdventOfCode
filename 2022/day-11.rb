...
def parse_input(path)
  ...
  x.each do |y|
    ...
    monkey = m.scan(/(\d)/).flatten!.map(&:to_i)
    start = s.scan(/(\d+)/).flatten!.map(&:to_i)
    ...
    test = t.scan(/(\d+)/).flatten!.map(&:to_i)
    test_t = r_t.scan(/(\d)/).flatten!.map(&:to_i)
    test_f = t_f.scan(/(\d)/).flatten!.map(&:to_i)
