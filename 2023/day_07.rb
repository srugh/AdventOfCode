...
def parse_file(path)
  ...
  File.read(path).split("\n").map(&:split).each_with_index do |line, idx|
    hand, bid = line
...
def solve_part1(hands)
  ...
  hands.each do |k, v|
    ...
    c_vals = cards.chars.map do |c|
      $CARD_VALUES[c]
    end
    hands[k]['card_values'] = c_vals
    sorted_counts = card_counts.sort_by { |_key, value| -value }.to_h

    has_trips = false
    has_pair = false
    sorted_counts.each_value do |count|
      if count == 5
        hands[k]['hand'] = 'AAAAA'
        break
      elsif count == 4
  ...
  sorted_multi_criteria = hands.sort_by do |_name, data|
    [data['hand_value'], data['card_values'][0], data['card_values'][1], data['card_values'][2],
     data['card_values'][3], data['card_values'][4]]
  end.to_h

  sorted_multi_criteria.each_with_index do |(_key, vals), rank|
...
def solve_part2(hands)
  ...
  sorted_multi_criteria.each_with_index do |(key, vals), _rank|
    p hands[key] if vals['cards'].count('J').positive?
  end
  sorted_multi_criteria.each_with_index do |(_key, vals), rank|
    score += vals['bid'] * (rank + 1)
  end
