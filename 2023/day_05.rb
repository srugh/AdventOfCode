...
def parse_file(path)
  ...
  chunks.shift.last.split(': ')[1].split.map(&:to_i).each do |seed|
    seeds[seed]
  end

  temp = chunks.shift
  temp.shift
  mappings['seed_to_soil'] = temp.map { |str| str.split.map(&:to_i) }

  temp = chunks.shift
  temp.shift
  mappings['soil_to_fertilizer'] = temp.map { |str| str.split.map(&:to_i) }

  temp = chunks.shift
  temp.shift
  mappings['fertilizer_to_water'] = temp.map { |str| str.split.map(&:to_i) }

  temp = chunks.shift
  temp.shift
  mappings['water_to_light'] = temp.map { |str| str.split.map(&:to_i) }

  temp = chunks.shift
  temp.shift
  mappings['light_to_temp'] = temp.map { |str| str.split.map(&:to_i) }

  temp = chunks.shift
  temp.shift
  mappings['temp_to_humidity'] = temp.map { |str| str.split.map(&:to_i) }

  temp = chunks.shift
  temp.shift
  mappings['humidity_to_location'] = temp.map { |str| str.split.map(&:to_i) }
