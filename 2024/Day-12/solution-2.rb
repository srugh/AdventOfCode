...
def find_regions(grid)
    ...
    grid.each_with_index do |row, row_idx|
        row.each_with_index do |plot, col_idx|
            if !plot_in_a_region.include?([row_idx, col_idx])
                visited = Set.new
                ...
                while !queue.empty?
                    ...
                    directions.each do |dir|
                        ...
                        if is_valid?(plot, cur_pos, next_pos, plot_in_a_region, grid) && !visited.include?(next_pos)
                            queue << next_pos
...
def is_valid?(plot, cur_pos, next_pos, plot_in_a_region, grid)
...
def calc_fencing(regions, grid)
    ...
    regions.each do |region|
        ...
        region[1].each do |cur_pos|
            ...
            plot = grid[cur_pos[0]][cur_pos[1]]
            directions.each do |dir|
                next_pos = [cur_pos[0] + dir[0], cur_pos[1] + dir[1]]
                has_border = calc_border?(cur_pos, next_pos, grid)

                if has_border && dir == [-1,0]
                
                    left_cell = [cur_pos[0],cur_pos[1]-1]
         
                    if region[1].include?(left_cell)
                 
                        if calc_border?(left_cell, [left_cell[0] + dir[0], left_cell[1] + dir[1]], grid )
                ...
                if has_border && dir == [0,-1]
                   
                    top_cell = [cur_pos[0]-1,cur_pos[1]]
         
                    if region[1].include?(top_cell)
                      
                        if calc_border?(top_cell, [top_cell[0] + dir[0], top_cell[1] + dir[1]], grid )
