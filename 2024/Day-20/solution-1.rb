...
def find_initial_positions(grid)
  ...
  grid.each_with_index do |row, r_idx|
    row.each_with_index do |col, c_idx|
      ...
      return [start_pos, final_pos] if start_pos.size.positive? && final_pos.size.positive?
    end
  end
end
...
def generate_cheat_maps(grid, path)
  ...
  path.each do |pos|
    dirs.each do |dir|
      ...
      # Boundary checks
      if next_pos_x_2[0].between?(0,
                                  grid.size - 1) && next_pos_x_2[1].between?(0,
                                                                             grid[0].size - 1) && is_wall?(grid,
                                                                                                           next_pos)
...
def has_adjacent_track?(grid, pos)
  ...
  dirs.any? do |dir|
    ...
    if adjacent_pos[0].between?(0, grid.size - 1) && adjacent_pos[1].between?(0, grid[0].size - 1)
      ['.', 'S', 'E'].include?(grid[adjacent_pos[0]][adjacent_pos[1]])
    else
      false
    end
  end
...
def calc_cheat_paths(grid, remove_walls, start, final, path_size, _input_file)
  loc_grid = []
  fast_cheats = 0
...
def calc_fast_cheat_paths(grid, input_file)
  ...
  calc_cheat_paths(grid, remove_walls, start_pos, final_pos, path.size - 1, input_file)
end
