...
def calc_code_complexity(code, _num_pad, _dir_pad, _num_min_dists, _dir_min_dists, num_min_paths, _dir_min_paths)
  num_path = []
...
def calc_min_dists(grid)
  ...
  grid.each_with_index do |start_row, sr|
    start_row.each_with_index do |_start_col, sc|
      grid.each_with_index do |end_row, er|
        end_row.each_with_index do |_end_col, ec|
          next if (sr == er && sc == ec) || grid[sr][sc] == '#'
...
def in_grid?(grid, pos)
  r, c = pos
  return true if r >= 0 && r < grid.size && c >= 0 && c < grid[0].size
...
def bfs_path(grid, start_pos, end_pos)
  ...
  while queue.size.positive?
    ...
    dirs.each do |dir|
      ...
      # puts "next pos"
      # p next_pos
      next unless in_grid?(grid, next_pos)
...
def create_input_grids
  ...
  num_pad.push(%w[7 8 9])
  num_pad.push(%w[4 5 6])
  num_pad.push(%w[1 2 3])
  num_pad.push(['#', '0', 'A'])
