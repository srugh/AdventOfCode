...
def parse_input(path)
  ...
  balls = temp.shift.join.split(',').map(&:to_i)
  cards = []

  temp.each_with_index do |c, _c_num|
    card_r = []
    card_c = Array.new(5) { [] }
...
def solve_part1(balls, cards)
  winner = -1
  ball = -1
  while balls.size.positive? && winner == -1
    ball = balls.shift
    cards.each_with_index do |card, idx|
      card.each do |row|
        row.delete_if { |element| element == ball }
        winner = idx if row.empty?
      end
...
def solve_part2(balls, cards)
  last = -1
  ball = -1
  winners = Set.new
  while balls.size.positive? && winners.size != cards.size
    ball = balls.shift
    cards.each_with_index do |card, idx|
...
# part_1 = solve_part1(balls.clone, cards.clone)

part_2 = solve_part2(balls, cards)
