...
def solve_part1(nanobots)
  key, r = nanobots.max_by { |_key, value| value }
  x, y, z = key

  count = 0
  nanobots.each_key do |(dx, dy, dz)|
    count += 1 if (x - dx).abs + (y - dy).abs + (z - dz).abs <= r
  end
  count
end
...
puts "part 1: #{solve_part1(nbs)}"
puts "part 2: #{solve_part2(nbs)}"
