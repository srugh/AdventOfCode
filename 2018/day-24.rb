...
class Army
    ...
    def total_damage(weaknesses, immunities)
        ...
        return @units * @attack_damage * multiplier
    end
    ...
    def get_best_target
        ...
        return @targets[index] if max_damage > 0
...
def parse_input(input)
    ...
    lines.each do |line|
        ...
        faction = "immunity"
        name = faction + "_" + immune.size.to_s
        army = Army.new(faction, units, hit_points, attack_damage, attack_type, initiative, weaknesses, immunities, name )
        immune.push(army)
    end
    ...
    lines.each do |line|
        ...
        faction = "infection"
        name = faction + "_" + infection.size.to_s
        army = Army.new(faction, units, hit_points, attack_damage, attack_type, initiative, weaknesses, immunities, name )
        infection.push(army)
    end
    [immune, infection]
...
def fight(immune_army, infection_army)
    ...
    while immune_army.size > 0 && infection_army.size > 0
...
def attack_phase(immune_army, infection_army)
    ...
    while imm_idx < immune_army.size && inf_idx < infection_army
        if imm_idx == immune_army.size
            imm_remaining = false
        end
        if inf_idx == infection_army.size
            inf_remaining = false
        end
...
def target_phase(immune_army, infection_army)
    ...
    immune_army.each_with_index do |imm_group, idx|
    ...
    infection_army.each_with_index do |inf_group, idx|
