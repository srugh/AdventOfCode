...
def move(unit_id, units, walls)
  ...
  unit = units[unit_id]
  start = unit["location"]
  foe_locs = units.select do |id, data|
    data["race"] != unit["race"]
  end.map { |id, data| data["location"] }
  ...
  in_range_squares.each do |target|
    moves.push(find_moves(start, target, all_unit_locs, walls))
  end
...
def game_over?(units)
  return units.count { |_id, data| data["race"] == "elf" } == 0 ||
    units.count { |_id, data| data["race"] == "goblin" } == 0
end
...
def solve_part_2(grid)
  ...
  loop do 
    ...
    if won 
      ...
      if min_power_with_flawless_win - max_power_with_loss == 1
        return score
      else 
        elf_power = (min_power_with_flawless_win + max_power_with_loss) / 2
        elf_power += 1 if (min_power_with_flawless_win + max_power_with_loss) % 2 != 0
      end
    else
